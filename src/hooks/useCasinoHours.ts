...
/* 
  This hook is designed to intercept the 403 sent from ships-service in the event of the casino being closed
  and the middleware blocking the call. It such a case it will trigger the onTrigger func passed in
*/
const useCasinoHours = () => {
  ...
  useEffect(() => {
    const newInterceptor = axios.interceptors.response.use(
      ...
      // on a failed response, we check to see if the status code is a 403, and if it contains the casino closed headers
      (error) => {
        if (axios.isAxiosError(error)) {
          const axiosError = error as AxiosError;
          if (
            axiosError.response?.status === 403 &&
            axiosError.response?.headers?.['x-casino-closed']
          ) {
            onToggle?.();
          }
        }
